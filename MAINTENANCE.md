...
- **`add_image_compression_control.py`** — removed 2026-09-02. Upstream Turnip now
  implements `VK_EXT_image_compression_control(_swapchain)` itself, and more completely than
  we did: `40f9c3bd` advertises both, `86509c12` honours `VK_IMAGE_COMPRESSION_DISABLED_EXT`
  in image creation (including the explicit-DRM-modifier list case, which our version did not
  handle), `9d2be1b6` reports compression in `vkGetImageSubresourceLayout2` and `6699db0d` in
  `vkGetPhysicalDeviceImageFormatProperties2`. The shared-runtime half landed too — `e39a340f`
  forces linear on a DISABLED request and `1320fc8b` deep-copies the control struct for
  deferred AHB/ANB images. Note upstream guards the `_swapchain` half with
  `#ifdef TU_USE_WSI_PLATFORM`, which an `-Dplatforms=android` build does not set; that is
  harmless here, because the Android loader forwards `VkImageCompressionControlEXT` into the
  gralloc-usage query regardless, and that query is the mechanism this driver actually relies
  on.
