...
| `add_image_compression_control.py` | `tu_device.cc` ext table + features; `tu_image.cc` UBWC choice + subresource layout; `tu_formats.cc` format query | **needed** | Implements VK_EXT_image_compression_control **and** `_swapchain`. The sanctioned form of the UBWC hack. **Coupled to `add_ubwc_swapchain_usage.py`, which must run first** - the loader chains the request onto the gralloc-usage query that script already patches, so the DISABLED check lives there. Only DISABLED changes behaviour; absent/DEFAULT keeps the driver default, or apps that never ask (Eden) lose compression. UBWC is lossless so fixed-rate is always NONE. |
